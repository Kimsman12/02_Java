...
public class ToyFactory {
	...
	public ToyFactory() { // 기본생성자
		...
		toySet.add(new Toy("키시미시", 5, 15000, "핑크색", "19940505", addMaterials(1,2)));
		...
		toySet.add(new Toy("허기위기", 5, 12000, "파란색", "19950805", addMaterials(1,4)));
	 ...
	 * 
	 * 가변인자 작성법 : 자료형...변수명
	 * -> 매개변수의 수가 정확히 몇개인지 특정할 수 없을때 사용
	 */
	public Set<String> addMaterials( Integer...materials/*List<Integer> materials*/) {
		...
		for(Integer meterialkey : materials) {
			// 맵에서 재료 고유 번호(key)에 대응하는 재료명(value)를 가져와서
			// addMaterials 에 추가
			String materialValue = materialMap.get(meterialkey);
			
			// map에 없는 key를 입력하며 value 를 얻어왓을 때 == null
...
public void displayMenu() {
	...
	public void createNewToy() throws Exception {
		...
		while(true) {
			...
			try {
				...
				int materialKey = Integer.parseInt(input);
			...
			} catch(NumberFormatException e) { // 재료번호 입력 시 숫자나 q 가 아닌 경우 예외처리
	...
	public void addMaterial() throws Exception {
		...
		if(materialMap.containsKey(key)) { // materialMap에 이미 존재하는 Key인지 확인
			
			System.out.println("이매 해당 키에 재료가 등록되어 있습니다.");
			System.out.print("덮어쓰시겠습니까?(Y/N) : ");
			String input = sc.next();
			
			if(input.equalsIgnoreCase("Y")) {
				// equalsIgnoreCase : 대문자인지 소문자인지 구분하지 않고 문자열 값 자체만 구분
				materialMap.put(key, material); // 같은 key로 추가 시 value 를 덮어쓰기
			} else {
	...
	public void displayToysByManufactureDate() {
		...
		toyList.sort(Comparator.comparing(Toy::getManifactureDate));
	...
	public void displayToysByAge() {
		...
		for(Toy toy : toySet) {
			...
			// -> Map에 해당 연령의 리스트가 없는 경우에만 새로운 List를 생성하여 추가
			
			toysByAge.get(age).add(toy);
			// toysByAge.get(age) -> age(Key)에 맞는 Value 반환 -> List
		...
		for(Entry<Integer, List<Toy>> entry : toysByAge.entrySet()) {
			...
			List<Toy> toyList = entry.getValue();// 해당 entry의 value 를 얻어옴
